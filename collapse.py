...
#print(artifact_datasets)

# expert1 in this code represents the gold standard data used for player feedback
# which in the db is referred to as expert2.  the index in this code for expert 1 is 0.
count = 0
for line in Lines:
    count += 1
    if count > 1:
...
#        print(line)
        ...
        # print ('ad,player,uncut',artifact_datatype,player_id,label_answer)
...
def build_crowd_answer(cohort_ratings,cohort,players,megatile,plaque_type,deltas):
      # generatie collapse pass zero (no collapsing yet - just a list of all markers from cohort)
      ...
      while collapse_tally != 0:
        ...
        for indexed_rating1, indexed_rating2 in itertools.combinations(enumerate(cohort_ratings[pass_count]), 2):
          ...
          # print('indexed_rating',indexed_rating1,indexed_rating2)
...
def run_sim(deltas,response_thresholds,cohort_size,samples):
  ...
  #print('new_tally is more than sum of remove and keep because it includes null ratings')
  #print('keep:',keep_tally/total_tally,'remove:',remove_tally/total_tally)
  #print('remove tally',remove_tally,'keep tally',keep_tally,'new tally',new_tally)

  # compare to expert answers
  #print('Compare Crowd to Expert Answers')
  gtp = 0
  gfp = 0
  gfn = 0
  gf1 = 0
...
#    print(deltas[0],deltas[1],response_thresholds[0],response_thresholds[1],cohort_size,samples,
...
#      gtp_type[1],gfp_type[1],gfn_type[1],f1_grand_caa)
    ...
    #print('total samples:',gtp+gfp+gfn)

# -------
# simulation stats

# best 4/10
# best overall: [4,70], 0.25, 30 = .726
# 1,2,0.04
...
# note: if cohort_size = 1, then routine will free if sample more than 30 because sampling
#   without replacement

# deltas = [4,70] # distance thresholds for Cored, CAA
# response_threshold = 0.25 # responder percentage threshold above which to retain a crowd answer

cohort_size = 15 # number of players per sampled cohort
samples = 20 # number of cohorts to sample for each megatile

delta_min = [2,2]
delta_max = [4,4]
delta_step = [1,1]
response_min = [0.11,0.03]
response_max = [0.14,0.06]
response_step = [0.01,0.01]

# run_sim(deltas,response_threshold,cohort_size,samples)
# run_sim([4,70],0.25,30,1)

answers=[]
maxval=0

#print('deltas,response_thresholds,cohort_size,samples,n,tp,fp,fn,f-score')
#for delta_cored in numpy.arange(delta_min[0],delta_max[0],delta_step[0]):
#  for delta_caa in numpy.arange(delta_min[1],delta_max[1],delta_step[1]):
#    for response_cored in numpy.arange(response_min[0],response_max[0],response_step[0]):
#      for response_caa in numpy.arange(response_min[1],response_max[1],response_step[1]):
#        deltas = [delta_cored,delta_caa]
#        response_thresholds = [response_cored,response_caa]
#        fscore = run_sim(deltas,response_thresholds,cohort_size,samples)
#        if fscore > maxval:
#          maxval = fscore
#          print('****** ',maxval)

print('deltas,response_threshold,cohort_size,samples,n,tp,fp,fn,f-score')
...
#  response_threshold_cored = 0.12
  if (response_threshold_cored * cohort_size) < 1:
    response_threshold_cored = 1/cohort_size
#  response_threshold_caa = 0.04
#  if (response_threshold_caa * cohort_size) < 1:
#    response_threshold_caa = 1/cohort_size

  
  fscore = run_sim([2,2],[response_threshold_cored,response_threshold_caa],cohort_size,50)
  print(response_threshold_cored * cohort_size, response_threshold_caa * cohort_size)
  if fscore > maxval:
    maxval = fscore
    print('****** ',maxval)
